...
public final class SimpleChat extends org.bukkit.plugin.java.JavaPlugin {
    ...
    public void onEnable() {
        ...
        // 注册命令
        SimpleChatCommand command = new SimpleChatCommand(configManager);
        Objects.requireNonNull(getCommand("simplechat")).setExecutor(command);
        Objects.requireNonNull(getCommand("simplechat")).setTabCompleter(command);
        ...
        if (Bukkit.getPluginManager().getPlugin("PlaceholderAPI") == null && !configManager.isIgnorePlaceholderAPI()) {
            getLogger().warning("这是一条建议：安装PlaceholderAPI插件以支持更多变量!（不安装也不影响插件基本功能）");
        }
    }
